...
boxes_CNN,X,Y=coord_CNN(boxes,size,248) #448
...
boxes_yolo=convert(248,boxes_CNN) #448
...
###### ANTIGUA
#Para 3 clases
#y_pred = tensor(7*7,(3+1*5)) --> output del modelo
#y_true debe ser un tensor equivalente a y_pred generado con las anotaciones
...
#     to the network prediction in the loss function.
#     '''
#     #QUITAMOS B POR AHORA: grid = np.array([ [[float(x),float(y)]]*B for y in range(7) for x in range(7)])
#     #grid = np.array([ [[float(x),float(y)]] for y in range(7) for x in range(7)])
    
#     # pred_boxes = K.Reshape(y_pred[...,3:], (-1,7*7,B,5))
#     pred_boxes = K.reshape(y_pred[...,3:], (-1,7*7,5))#245
#     print(pred_boxes.shape)
#     true_boxes = K.reshape(y_true[...,3:], (-1,7*7,5))#245
#     print(true_boxes.shape)
#         #axis0=..., axis1=49, axis2=5
    
#     # probabilidad de que haya un objeto
#     y_pred_conf = pred_boxes[...,4]
#     y_true_conf = true_boxes[...,4]
#         #axis0=..., axis1=49, axis2=1
       
#     ### xy_loss--------------------------------------
#     y_pred_xy   = pred_boxes[...,0:2] #+ K.variable(grid) 
#     y_true_xy   = true_boxes[...,0:2]
#         #axis0=..., axis1=49, axis2=2
...
#     #+ 0.5(K.sum(K.square(y_true_conf*iou - y_pred_conf)y_true_conf, axis=-1))
