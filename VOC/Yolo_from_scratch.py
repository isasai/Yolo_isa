...
x=Conv2D(16, (3,3),padding='same')(i) #Conv1  
x=keras.layers.LeakyReLU(alpha=0.3)(x)
x=MaxPooling2D((2, 2))(x) ###1 ->124
x=Conv2D(32, (3, 3),padding='same')(x) #Conv2
x = keras.layers.LeakyReLU(alpha=0.3)(x)
x=MaxPooling2D((2, 2))(x) ###2 -> 62
x=Conv2D(16, (1,1),padding='same')(x) #Conv3
x=Conv2D(128, (3, 3),padding='same')(x) #Conv4
x=Conv2D(16, (1,1),padding='same')(x) #Conv5
x=Conv2D(128, (3,3),padding='same')(x) #Conv6
x = keras.layers.LeakyReLU(alpha=0.3)(x)
x=MaxPooling2D((2, 2),padding='same')(x)
x=Conv2D(32, (1,1),padding='same')(x) #Conv3
x=Conv2D(256, (3, 3),padding='same')(x) #Conv4
x=Conv2D(32, (1,1),padding='same')(x) #Conv5
x=Conv2D(256, (3,3),padding='same')(x) #Conv6
x = keras.layers.LeakyReLU(alpha=0.3)(x)
x=MaxPooling2D((2, 2),padding='same')(x) ###3 -> 31
x=Conv2D(64, (1,1),padding='same')(x) #Conv7
x=Conv2D(512, (3, 3),padding='same')(x) ##Conv8
x=Conv2D(64, (1,1),padding='same')(x) #Conv9
x=Conv2D(512, (3, 3),padding='same')(x)
x=Conv2D(128, (1,1),padding='same')(x) #Conv9
x=Conv2D(1000, (1,1),padding='same')(x) #Conv10
x=Flatten()(x)
x=Dense(250,activation='sigmoid')(x)
x=Dropout(0.5)(x)
x=Dense(7*7*(1*5+6),activation='sigmoid')(x)
x=Reshape((7*7,(1*5+6)))(x)
...
def loss_yolo(y_pred,y_true):
    ...
    pred_boxes = K.reshape(y_pred[...,6:], (-1,7*7,5))#245
    true_boxes = K.reshape(y_true[...,6:], (-1,7*7,5))#245
    pred_boxes.shape
    true_boxes.shape
    ...
    y_pred_class = K.reshape(y_pred[...,0:6], (-1,7*7,6))
    y_true_class = K.reshape(y_true[...,0:6], (-1,7*7,6))
    y_true_class.shape
...
model=Model(i,x)
model.compile(optimizer=optimizers.Adam(lr=0.01, beta_1=0.9, beta_2=0.999, decay=0.0005),
              loss=loss_yolo,
              metrics=['acc']) 
...
history=model.fit(x_train,y_true, batch_size=60, epochs=1, validation_data=(x_val,y_val),verbose=1)
...
import pickle
with open('C:/Users/adrin/Desktop/isa/Yolo_isa-master/Yolo_isa-master/y_pred_VOC5.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
